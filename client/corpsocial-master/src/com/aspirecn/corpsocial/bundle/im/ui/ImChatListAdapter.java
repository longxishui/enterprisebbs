...
public class ImChatListAdapter extends BaseAdapter {
    ...
    public ImChatListAdapter(Context context, List<ImMainTabItem> imMainTabItems) {
    ...
    public View getView(int position, View convertView, ViewGroup arg2) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(mContext).inflate(R.layout.im_main_tab_item, null);
    ...
    private void bindingData(ViewHolder holder, int position) {
        ...
        if (item.isSetMoveToTop()) {// 已置顶
            holder.mItemLayout.setBackgroundResource(R.drawable.listview_item_bg_selector2);
        } else {
            holder.mItemLayout.setBackgroundResource(R.drawable.listview_item_bg_selector);
        }

        // 头像
        if (item.getItemType() == ItemType.CorpGroupChat) {// 公司群
            holder.mHeadPortraitTv.setVisibility(View.VISIBLE);
            holder.mHeadPortraitIv.setVisibility(View.INVISIBLE);
            ...
            holder.mHeadPortraitTv.setVisibility(View.INVISIBLE);
            holder.mHeadPortraitIv.setVisibility(View.VISIBLE);

            User user = getUser(item.getItemId());
            if (user != null) {
                ImageLoader.getInstance().displayImage(user.getUrl(), holder.mHeadPortraitIv, options);
            }
            holder.mHeadPortraitTv.setText("");
            bindMessageView(item, holder);
        ...
        } else if (item.getItemType() == ItemType.PubAccountCommonChat) {//普通公众号会话
            holder.mHeadPortraitTv.setVisibility(View.INVISIBLE);
            holder.mHeadPortraitIv.setVisibility(View.VISIBLE);
            holder.mHeadPortraitTv.setText("");
            ...
            DisplayImageOptions options = new DisplayImageOptions.Builder()
                    ...
                    .showImageForEmptyUri(R.drawable.addrbook_contact)
                    .build();
            ImageLoader.getInstance().displayImage(item.getHeadImg(), holder.mHeadPortraitIv, options);

            bindMessageView(item, holder);

        } else if ((item.getItemType() == ItemType.PubAccountServiceChat)) {//服务公众号会话
            holder.mHeadPortraitTv.setVisibility(View.INVISIBLE);
            holder.mHeadPortraitIv.setVisibility(View.VISIBLE);
            holder.mHeadPortraitTv.setText("");
    ...
    private void bindMessageView(ImMainTabItem item, ViewHolder holder) {
        ...
        // 最新消息
        holder.mNewMsgTv.setText(item.getIntro());
    }
